...
% Coefficients:  1.0 0.
% Perturbed parameters: a in [0.9,1.1], b in [2.7,3.2], c in [1.1,1.6]
...
function jetEngine_Pontryagin()
	...
	% 	The uncertain parameter b belongs to [2.7,3.2];
	...
	f(1) = (-u(1))*x(2)-u(3)*x(1)*x(1)-0.5*x(1)*x(1)*x(1)-0.5;
	f(2) = u(2)*x(1)-x(2);
	...
	cHeader = { 'a' 'b' 'c'	 };
	pontryagin(maximize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'jetEngine_Pontryagin_max.csv',doPlot);
	pontryagin(minimize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'jetEngine_Pontryagin_min.csv',doPlot);
