...
void*param)
{
 ...
 yp[0] = -b*y[0] + y[0]*y[0]*y[1] + a;
 yp[1] = c*y[0] - y[0]*y[0]*y[1];
...
int m=0;
FILE *fp = fopen("Brusselator_Pontryagin_max.csv","r");
char ch;
while (!feof(fp)){
  ch = fgetc(fp);
  if(ch == '\n')
...
iVector t_(m);
string line;
ifstream file("Brusselator_Pontryagin_max.csv");
getline(file,line);
int j;
for (int i=0;i<m;i++){
   getline(file,line);
...
VNODE*Solver;
for (int i=0; i<m ; i++){
 myad= new FADBAD_AD(n,VDP,VDP,&p[i]);
...
m=0;
fp = fopen("Brusselator_Pontryagin_min.csv","r");
while (!feof(fp)){
  ch = fgetc(fp);
  if(ch == '\n')
    m++;
...
ifstream file2("Brusselator_Pontryagin_min.csv");
...
t=0.0;
for (int i=0; i<m ; i++){
 myad= new FADBAD_AD(n,VDP,VDP,&p2[i]);
 Solver= new VNODE(myad);
