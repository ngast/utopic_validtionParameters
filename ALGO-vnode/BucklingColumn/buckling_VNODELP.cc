...
void*param)
{
 ...
 yp[0] = a*y[1];
 yp[1] = b*y[0] -y[0]*y[0]*y[0] - 2.0*y[1] + c;
...
int m=0;
FILE *fp = fopen("Buckling_Pontryagin_max.csv","r");
char ch;
while (!feof(fp)){
  ch = fgetc(fp);
  if(ch == '\n')
...
iVector t_(m);
string line;
ifstream file("Buckling_Pontryagin_max.csv");
getline(file,line);
int j;
for (int i=0;i<m;i++){
   getline(file,line);
...
m=0;
fp = fopen("Buckling_Pontryagin_min.csv","r");
while (!feof(fp)){
  ch = fgetc(fp);
  if(ch == '\n')
    m++;
...
ifstream file2("Buckling_Pontryagin_min.csv");
