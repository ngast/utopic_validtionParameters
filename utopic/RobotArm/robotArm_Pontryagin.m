%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Reachability analysis of the system robotArm with 3 uncertain parameters %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Automatically generated from robotArm.
% Number of species: 4.
% Number of reactions: 4.
% Coefficients:  0 0 1.0 0.
% Perturbed parameters: a in [0.9,1.1], b in [1.2,1.8], c in [3.2,4.2]
...
function robotArm_Pontryagin()
	...
	% Plot flag
	doPlot = 1;
	% 	The uncertain parameter a belongs to [0.9,1.1];
	% 	The uncertain parameter b belongs to [1.2,1.8];
	% 	The uncertain parameter c belongs to [3.2,4.2];
	...
	minimize = 0;
	delta = -1.0; % threshold
	max_iter = 1200; % maximal number of algorithm iterations
	step = 10.0;
	...
	f(1) = x(3);
	f(2) = u(3)*x(4);
	f(3) = ((-u(1))*x(2)*x(3)*x(4)-u(1)*x(1)-u(2)*x(3)+4)/(x(2)*x(2)+1);
	f(4) = x(2)*x(3)*x(3)-x(2)-x(4)+1;


	initpontryagin(f,p,x,u);

	cHeader = { 'a' 'b' 'c'	 };
	pontryagin(maximize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'robotArm_Pontryagin_max.csv',doPlot);
	pontryagin(minimize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'robotArm_Pontryagin_min.csv',doPlot);
