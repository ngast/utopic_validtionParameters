%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Reachability analysis of the system vandelPol with 3 uncertain parameters %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Automatically generated from vandelPol.
% Number of species: 2.
% Number of reactions: 2.
% Coefficients:  1.0 0.
% Perturbed parameters: a in [0.5,1.0], b in [1.0,1.2], c in [0.8,1.2]
...
function vanderPol_Pontryagin()
	...
	% Plot flag
	doPlot = 1;
	% 	The uncertain parameter a belongs to [0.5,1.0];
	% 	The uncertain parameter b belongs to [1.0,1.2];
	% 	The uncertain parameter c belongs to [0.8,1.2];
	...
	minimize = 0;
	delta = -1.0; % threshold
	max_iter = 100; % maximal number of algorithm iterations
	step = 10.0;
	...
	f(1) = u(1)*x(2);
	f(2) = -x(2)+(-u(2))*x(1)+(-u(3))*x(1)*x(1)*x(2);


	initpontryagin(f,p,x,u);

	cHeader = { 'a' 'b' 'c'	 };
	pontryagin(maximize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'vanderPol_Pontryagin_max.csv',doPlot);
	pontryagin(minimize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'vanderPol_Pontryagin_min.csv',doPlot);
