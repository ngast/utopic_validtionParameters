...
%     x(3) = A(b1!1,b2).B(b!1)
%     x(4) = A(b1,b2!1).B(b!1)
%     x(5) = A(b1!1,b2!2).B(b!2).B(b!1)


function replicatedBindingSite2Asym_Pontryagin()
	...
	% Plot flag
	doPlot = 1;
	% 	The uncertain parameter kBind1 belongs to [9.99,10.0];
	% 	The uncertain parameter kBind2 belongs to [9.89,9.9];
	...
	%Initial condition
	x0=[  0.0343, 0.0979, 0.1922, 0.3365, 0.3391 ];
	...
	f(1) = (-u(1))*x(1)*x(2)+(-u(2))*x(1)*x(2)+u(3)*x(3)+0.1*x(4);
	f(2) = (-u(1))*x(1)*x(2)+(-u(2))*x(1)*x(2)+(-u(1))*x(2)*x(4)+u(3)*x(3)+(-u(2))*x(2)*x(3)+0.1*x(4)+u(3)*x(5)+0.1*x(5);
	f(3) = u(1)*x(1)*x(2)+(-u(3))*x(3)+(-u(2))*x(2)*x(3)+0.1*x(5);
	f(4) = u(2)*x(1)*x(2)+(-u(1))*x(2)*x(4)+(-0.1)*x(4)+u(3)*x(5);
	f(5) = u(1)*x(2)*x(4)+u(2)*x(2)*x(3)+(-u(3))*x(5)+(-0.1)*x(5);


	initpontryagin(f,p,x,u);

	cHeader = { 'kBind1' 'kBind2' 'kUnbind1'	 };
	pontryagin(maximize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'replicatedBindingSite2Asym_Pontryagin_max.csv',doPlot);
	pontryagin(minimize,vcoeff,max_iter,delta,uVars,cHeader,gamma,tspan,tspanRev,x0,u_max,u_min,'replicatedBindingSite2Asym_Pontryagin_min.csv',doPlot);
